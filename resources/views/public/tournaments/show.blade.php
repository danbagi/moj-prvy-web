...
<div x-data="{tab: 'schedule'}" x-init="setInterval(() => fetch('/tournaments/{{ $tournament->slug }}/poll').then(() => location.reload()), 10000)">
    ...
    <section class="mt-4" x-show="tab==='schedule'">
        @foreach($schedule as $m)
            <div class="bg-white p-3 rounded mb-2">{{ $m->homeTeam?->name }} vs {{ $m->awayTeam?->name }} <span class="text-xs">{{ $m->status }}</span></div>
        @endforeach
    </section>
    ...
    <section class="mt-4" x-show="tab==='standings'">
        <a class="underline text-sm" href="/tournaments/{{ $tournament->slug }}/exports/standings.csv">Export CSV</a>
